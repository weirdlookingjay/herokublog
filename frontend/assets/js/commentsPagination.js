...
button.addEventListener("click", function(event) {
    ...
    pages.forEach(function(el) {
        el.addEventListener("click", function(event) {
            ...
            if (httpRequest) {
                httpRequest.open('POST', 'https://blog-coder.herokuapp.com/backend/ajax/showNextComments.php', true);
                //httpRequest.open('POST', 'http://herokublog.local/backend/ajax/showNextComments.php', true);
...
previousBtn.addEventListener("click", function(event) {
    ...
    if(currentNum > '1') {
        ...
        if (httpRequest) {
            httpRequest.open('POST', 'https://blog-coder.herokuapp.com/backend/ajax/showPreviousComments.php', true);
            //httpRequest.open('POST', 'http://herokublog.local/backend/ajax/showPreviousComments.php', true);
            httpRequest.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
                    document.querySelector('#posts').innerHTML = this.responseText;
                    currentPage.innerHTML = currentNum;
...
postLimit.addEventListener("change", function(e) {
    ...
    if (httpRequest) {
        httpRequest.open('POST', 'https://blog-coder.herokuapp.com/backend/ajax/jumpToComments.php', true);
        //httpRequest.open('POST', 'http://herokublog.local/backend/ajax/jumpToComments.php', true);
        httpRequest.onreadystatechange = function() {
            if (this.readyState === 4 && this.status === 200) {
                document.querySelector('#posts').innerHTML = this.responseText;
